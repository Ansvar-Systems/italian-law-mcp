/**
 * search_eu_implementations — Search EU directives/regulations with Italian implementation info.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_to?: number;
  community?: string;
  has_italian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS italian_statute_count,
  ...
  if (input.has_italian_implementation !== undefined) {
    sql += input.has_italian_implementation ? ` HAVING italian_statute_count > 0` : ` HAVING italian_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    italian_statute_count: number; primary_implementations: string | null;
  }
