/**
 * get_provision_eu_basis — Get EU legal basis for a specific Italian provision.
 */
...
export async function getProvisionEUBasis(
  ...
  // Try multiple variants for the provision reference
  ...
  let provision: { id: number; content: string } | undefined;

  for (const variant of searchVariants) {
    provision = db.prepare(
      'SELECT id, content FROM legal_provisions WHERE document_id = ? AND (provision_ref = ? OR section = ?)'
    ).get(resolvedId, variant, variant) as { id: number; content: string } | undefined;
    if (provision) break;
  }
